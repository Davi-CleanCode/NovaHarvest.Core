...
{
    public int Id { get; }
    public CropState State { get; private set; } = CropState.Empty; public string? SeedId { get; private set; }
    public int GrowthDay { get; private set; }
    public int WateredOnDay { get; private set; } = -1;
    public FarmPlot(int id) { Id = id; }
    public bool Plant(string seedId) { if (string.IsNullOrWhiteSpace(seedId) || State != CropState.Empty) return false; SeedId = seedId; GrowthDay = 0; WateredOnDay = -1; State = CropState.Planted; return true; }
